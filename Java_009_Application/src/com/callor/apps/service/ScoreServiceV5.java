...
public class ScoreServiceV5 {
	 ...
	 * 몇명의 학생, 일부과목 점수만 입력을 마치고 잠시 업무를 멈추고 싶을 때
	 * 현재 코드는 불가능하다
	...
	public void makeScores() {
		...
		for(int i = 0 ; i < intKor.length ; i++) {
			...
			for(int j = 0 ; i < subject.length ; j++) {
				 ...
				 * 이럴 경우 return type이 void이지만 return명령을 실행하여 method 코드를 중단할 수 있다.
				 */
				if(scores[j] == -1) {
					return;
				}
	...
	private Integer inputScore(String sub) {
		...
		while(true) {
			...
			if(score == -1) {
				...
				// continue 명령을 사용하는 대신 else를 사용하여 코드를 작
			} else {
			break;
			}
		}
	...
	/*
	 * 프로젝트를 만들고 컴파일을 수행하고 실행을 했을 때
	 * 
	 * 선언한 클래스는 코드만 존재하는 상태가 된다
	 * 이 클래스를 객체로 선언하고, 생성한 후에 여러가지 method를 호출하여 업무를 수행할 수 있다
	 * 
	 * method나 변수에 static을 부착하면 사용하지 않아도 method 코드나, 변수가 메모리에 상주하게 된다.
	 */
