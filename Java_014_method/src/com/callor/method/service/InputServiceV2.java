...
public class InputServiceV2 {
	...
	public Integer inputValue(String title, int start) {
		...
		String msgTitle = String.format("%s( %d 이상의 ~ )", title, null);
		while(true) {
	...
	/*
	 * 정수 범위에 대한 유효성 검사를 수행하는 method 이미 만들어진 inputValue(String title) method와 연동하여
	 * 사용하겠다
	 */
	public Integer inputValue(String title, int start, int end) {
		...
		while (true) {
			...
			// this.inputValue(String title) method가 return 해준 값이 null이 아니면 유효성 검사 수행
			if (intNum != null) {
				// 유효성 검사를 통과하지 못하면
	...
	/*
	 * exception이 발생했을 때 사용자에게 안내 메시지를 보여주고 다시 입력을 받도록 하기 위한 선 조치
	 */
