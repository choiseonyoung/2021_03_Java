 ...
 * 
 * 객체지향 프로그램언어(대표적으로 java에서는)
 * 한 개의 클래스(파일) 내에서 같은 이름의 함수(method)를 중복하여 선언할 수 있다.
 * 		한 개의 클래스에서 method를 중복 선언할 수 있다.
 * 단, 조건이 있는데
 * 		매개변수가 달라야 한다
...
public class NumberServiceV1 {
	 ...
	 * 		여기에서는 어떤 조건에 따라 null값을 return 해야 하는 경우도 있기 때문에
	...
	public Integer addNum(int num1, int num2) {
		...
		// 첫번째 방법
		// 어떤 연산을 수행하고 연산 결과에 따라 return 등을 수행해야 하는 경우
		...
		// 두 번째 방법
		// C언어와 같은 함수형 언어에서는
		// 		함수를 끝내는 return문은 한 번만 쓰자 라는 전통적인 패턴이 있다
		// 		전통적인 패턴을 지키는 코드 예제
		//	Integer sum = null;
		//	if ( (num1 + num2) == 0) {
		//		sum = num1 + num2;
		...
		// }
		// return null;
		// 이렇게 처리하는 방법이 비용이 많이 듦 (쓸데없는 연산 2번함)
