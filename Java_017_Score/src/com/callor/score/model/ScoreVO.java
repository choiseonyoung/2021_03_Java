 ...
 * 필요할 때마다 객체로 생성하고,
 * 데이터를 Setter하고 연산을 수행한 후 데이터를 Getter하는 방식으로 활용한다
 * 
...
public class ScoreVO {
	...
	// primitive형이 아닌 wrapper 클래스 형으로 선언할 때는
	// 		0으로 clear를 해 주자
	// (String형은 자동으로 null로 초기화됨)
	// 이러한 불편을 방지하기 위하여 primitive 형으로 만들기도 하지만 null 값을 취급하기가 어려운 단점이 발생한다.
...
//	private Float avg;
	...
	/*
	 * 캡슐화
	 * 외부에서 객체.getTtotal() method를 호출하면
	 ...
	 * 또는 관심갖지 않아도 상관없이 getTotal() method는 당연히 잘 계산된 총점결과를 알려준다는 것을 알고 있다.
	 * 
	 ...
	 * 
	 * 하지만, Object 클래스에 정의된 toString() method는 사용하는데 의미가 업다
	 * 
	 * 그래서 toString() method를
	 * VO 객체의 변수에 담긴 값들을 문자열로 만들어 return하는 코드로 다시 작성을 한다
	 * 이게 vo객체.toString() method를 호출하면
