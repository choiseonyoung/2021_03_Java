...
public class Number_02 {
	...
	public static void main(String[] args) {
		 ...
		 * 파일에 저장, 파일에서 읽기
		 * 네트워크(인터넷)등을 통하여 전송(공유) 등으로 데이터가 컴퓨터와 컴퓨터간에 이동할 때
		 * 데이터를 호율적이고, 오류가 나지 않도록 이동하는 다양한 방법이 있다
		 * 여러가지 방법이 취하는 많은 공통된 부분에 모든 데이터를 문자열화하여 전송하는 방법이 있다
		 * 숫자, 문자, 특수기호 등을 모두 문자열(ASCII 코드)로 전송하고,
		 * 	받는 곳에서 사용하기 전에 적절한 type으로 변경하여 사용한다
		 * 
		 * Java에서는 WrapperClass.parse*() method나 WrapperClass.ValuOf() method를 사용하여
